<template>
  <div class="p-8 pb-0 flex flex-col items-center justify-center">
    <input type="text" v-model="keyword" class="rounded-lg border-2 border-neutral-200 w-full outline-0 ring-0"
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 p-8">
    <div v-for="meal in searchResults" :key="meal.idMeal" class="bg-white shadow rounded-xl overflow-hidden">
      <router-link :to="{ name: 'mealDetails', params: { id: meal.idMeal } }">
        <img :src="meal.strMealThumb" :alt="meal.strMeal" class="h-52 w-full object-cover" />
        <div class="p-4">
